...
export class RossCarboniteSource extends TallyInput {
    ...
    /* MIKE Notes:
    We recieve a message for each transition change.  For example, if I go from cam2 -> cam1 pgm
    I get the message from cam 1 saying that it is live, then I get a message from cam 2 saying it is off now.
    We get no reminder of preview though, so we need to keep memory of that.

    The problem with the original implementation is that the process function is going through each camera.
    It sees 1 is live and 2 is off.  So it adds 1 tally and removes 2 tally, then sends a UI update
    However, then we get the message that 2 is off.  This gets confused and tries to set 2 live.
    ...
    */
    private processRossCarboniteTally(tallyObj) {
        ...
        //logger(`MIKE START PROCESS: ${labelAddress} -- ${tallyObj.address.toString()} -- ${tallyObj.tally1} -- ${tallyObj.tally2} `, 'info-quiet');
                ...
                if (device_sources[i].sourceId === this.source.id) { //this device_source is associated with the tally data of this source
                    if (device_sources[i].address === tallyObj.address.toString()) { //this device_source's address matches what was in the address field
                        if (device_sources[i].bus === 'onair') {
                            //logger(`MIKE PROCESS: ${labelAddress} -- ${tallyObj.address.toString()} -- ${tallyObj.tally1} -- ${tallyObj.tally2} `, 'info-quiet');
                            if (tallyObj.tally1) {
    ...
    private addRossCarboniteTally(busAddress, address) {
        let found = false;
        //logger(`MIKE ADD: ${busAddress} ${address} `, 'info-quiet');
    ...
    private removeRossCarboniteTally(busAddress, address) {
        for (let i = 0; i < this.tallydata_RossCarbonite.length; i++) {
            if (this.tallydata_RossCarbonite[i].address === address) {
                //logger(`MIKE REMOVE BEFORE: ${busAddress} ${address} -- ${this.tallydata_RossCarbonite[i].busses[0]} -- ${this.tallydata_RossCarbonite[i].busses.length}`, 'info-quiet');
                this.tallydata_RossCarbonite[i].busses = this.tallydata_RossCarbonite[i].busses.filter(bus => bus !== busAddress);
                //logger(`MIKE REMOVE AFTER: ${busAddress} ${address} -- ${this.tallydata_RossCarbonite[i].busses.length}`, 'info-quiet');
                this.updateRossCarboniteTallyData(this.tallydata_RossCarbonite[i].address);
            }
        }
    }
    ...
    private updateRossCarboniteTallyData(address) {
        ...
        let found = false;
        //logger(`MIKE UPDATE START:  ${address}`, 'info-quiet');
        ...
        for (let i = 0; i < device_sources.length; i++) {
            ...
            if (device_sources[i].address === address) {
                ...
                for (let j = 0; j < busses.length; j++) {
                    //MIKENOTE: sourceTypeID was looking for switcher bus address, not carbonite bus address
                    //let bus = RossCarboniteBusAdresses[this.source.sourceTypeId]?.find( (busAddress) => (busAddress.address === busses[j]));
                    let bus = RossSwitcherBusAddresses[this.source.sourceTypeId].find( (busAddress) => (busAddress.address === busses[j]));
                ...
                logger(`MIKE UPDATE: ${i} ${address} -- ${inPreview} -- ${inProgram} `, 'info-quiet');
                const b = [];
                if (inPreview) {
                    b.push("preview");
                }
